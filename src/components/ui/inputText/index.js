...
import Theme, { fontLarge, fontNormal, fontSmall, fontXLarge, fontXSmall, fontXXLarge, responsiveHeight, responsiveWidth } from '../../../utils/themeUtils';
...
const InputText = (props) => {
    ...
    if (props.input_sm) {
        ...
        inputTextStylesArray.push({ fontSize: fontSmall, paddingLeft: props.iconName ? 35 : 15 });
    } else if (props.input_md) {
        inputContainerStylesArray.push({
            height: responsiveHeight(6),
        });
        inputTextStylesArray.push({ fontSize: fontLarge, paddingLeft: props.iconName ? 45 : 15 });
    } else if (props.input_lg) {
        inputContainerStylesArray.push({
            height: responsiveHeight(8),
        });
        inputTextStylesArray.push({ fontSize: fontXXLarge, paddingLeft: props.iconName ? 55 : 15 });
    } else {
        inputContainerStylesArray.push({
            height: responsiveHeight(7),

        });
        inputTextStylesArray.push({ fontSize: fontNormal, paddingLeft: props.iconName ? 40 : 15 });
    }
    ...
    return (
        <>
            ...
            {props.error ? <Label normal align="center" color={Color.RED} mt={5}>{props.error}</Label> : null}
        </>
    );
}
